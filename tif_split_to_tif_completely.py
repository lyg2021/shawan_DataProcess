...
def read_tif(tif_image_path: str):
    ...
    # 这个循环是把这些波段的np数组堆叠起来
    for band_count in tqdm(range(1, big_tif.count+1)):
        if band_count == 1:
...
def split_tif(tif_array: np.ndarray, stride: int, crop_size: tuple):
    ...
    for y in tqdm(range(0, height+1, stride)):
...
def main(big_tif_image_path:str, crop_size:tuple, stride:int):
    # 输入路径
    ...
    for index, tif_array in enumerate(image_tif_array_list):
        ...
        save_array_as_tif(tif_array, os.path.join(
            save_image_path_root, f"{index:0>6d}.tif"), metadata)
...
if __name__ == "__main__":
    # 输入路径
    big_tif_image_path = "tif_image/GF6_PMS_E85.4_N44.6_20220514_L1A1120209504-MUX_fuse.tif"
